  ...
  return (
    <Layout headerType="alternate">
      ...
      <section id="rentals" className="py-16 px-6 bg-white scroll-mt-32">
        ...
        {decorItems && (
          <div ref={sliderRef} className="keen-slider" key={`${sortBy}-${sortOrder}`}>
            {filteredAndSortedItems.map((item: DecorItem) => (

              <div key={item.id} className="keen-slider__slide min-w-0 h-full">
                <div className="border rounded-2xl shadow-md hover:shadow-xl transition overflow-hidden bg-white">
                  <img
                    src={item.imageUrl}
                    alt={item.name}
